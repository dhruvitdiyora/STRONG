...
namespace CharityAPI.Services
{
    public class VolunteerServices : GenericRepository<Volunteer>, IVolunteer
    {
        ...
        {
            var vol = context.Volunteer.SingleOrDefault(x => x.VolunteerUserId == userid && x.OrganisationId == organisationId);
            return vol;
        ...
        {
            var volunteer = context.Volunteer.Where(x => x.OrganisationId == id && x.IsPublished == true).Include(x => x.VolunteerUser).ToList();
            return volunteer;
        }
