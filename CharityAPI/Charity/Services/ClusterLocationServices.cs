...
namespace CharityAPI.Services
{
    public class ClusterLocationServices : GenericRepository<ClusterLocations>, IClusterLocation
    {
        ...
        {
            var clusterLocation = context.ClusterLocations.Include(x=>x.RequirementType).Include(x => x.City).Include(x => x.State).Include(x=>x.Post).Where(x => x.IsPublished == true).ToList();
        ...
        public override bool Update(long id, ClusterLocations entity)
        {
            ...
            if(existingClusLocation != null)
            {
                ...
                existingClusLocation.ClusterDetails = entity.ClusterDetails;
