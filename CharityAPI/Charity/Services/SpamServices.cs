...
namespace CharityAPI.Services
{
    public class SpamServices : GenericRepository<Spam>, ISpam
    {
        ...
        public IEnumerable GetSpamByPostId(long id)
        {
            var spam = context.Spam.Where(x => x.PostId == id && x.IsPublished == true).Include(x => x.User);
            return spam;
        }
        //get spam by user id
        ...
        public Spam CheckSpam(long userid, long postId)
        {
            var spam = context.Spam.SingleOrDefault(x => x.UserId == userid && x.PostId == postId);
            return spam;
