...
namespace CharityAPI.Services
{
    public class AdminServices : GenericRepository<ApplicationUser>, IAdmin
    {
        ...
        public AdminServices(CharityAPIContext context, UserManager<ApplicationUser> userManager, RoleManager<IdentityRole> roleManager, IConfiguration configuration, IMailService mailService) : base(context)
        {
        ...
        public async Task<IdentityResult> RegisterAdmin(RegisterModel model)
        {
            ...
            if (result.Succeeded)
            {
                ...
                //var data = await users.ConfirmEmailAsync(userId, token);
                var data = await userManager.ConfirmEmailAsync(admin, confirmEmailToken);
                //
                ...
                //request.Body = $"<h1>You have successfully registered yourself with STRONG!</h1><h4>Your OTP is: 1234</h4><p><a href='{url}'>Click here</a> to verify your email</p>";
